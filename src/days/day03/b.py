...
class Day03PartBSolver(Day03Solver):
    def solve(self) -> AnswerType:
        ...
        for mult_match in matches:
            max_do = max((do_idx for do_idx in do if do_idx < mult_match.start()), default=-1)
            max_dont = max((dont_idx for dont_idx in dont if dont_idx < mult_match.start()), default=-2)
            if max_do > max_dont:
